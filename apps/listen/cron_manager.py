...
def _fire_cron(cron_id: str, prompt: str):
    """Submit a job to the listen server when a cron fires."""
    try:
        resp = httpx.post(
            f"{LISTEN_URL}/job",
            json={"prompt": prompt},
        ...
        if resp.status_code == 200:
            job_data = resp.json()
            logger.info(f"Cron {cron_id} fired → job {job_data.get('job_id', '?')}")
        else:
...
def trigger_cron(cron_id: str) -> bool:
    """Manually trigger a cron right now (for testing)."""
    cron = get_cron(cron_id)
    if not cron:
        return False
    _fire_cron(cron_id, cron["prompt"])
    return True
