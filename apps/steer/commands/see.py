...
def see(app, screen_idx, use_ocr, role, as_json):
    ...
    try:
        ...
        if screen_idx is not None and app is None:
            ...
            screen_capture.capture_app(app, screenshot_path)
            app_name = app
            elements = accessibility.walk(app)
            win_bounds = screen_capture.window_bounds(app)
            windows = [w.to_dict() for w in win_bounds]
            ...
            if front:
                app_name = front["name"]
                screen_capture.capture_app(app_name, screenshot_path)
                elements = accessibility.walk(app_name)
                win_bounds = screen_capture.window_bounds(app_name)
                windows = [w.to_dict() for w in win_bounds]
